...
const fetchPlatformHealth = async () => {
  try {
    // Route through /api/health for consistent dev proxying
...
const exportDashboardData = async (filters = {}) => {
  ...
  const response = await apiClient.get(
    `/api/admin/dashboard/export?${params}`,
    {
      responseType: "blob",
    },
  );
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-red-600 dark:text-red-300 text-sm mt-1">
                Failed to load this section. Please refresh the page or contact
                support.
              </p>
...
const AdminSectionSkeleton = ({ height = "h-64", title = true }) => (
  <div
    className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 ${height}`}
  >
...
const PlatformHealthMonitor = ({ healthData, dashboardData }) => {
  const getHealthStatus = () => {
    ...
    if (score >= 85)
      return { status: "good", color: "blue", icon: CheckCircleIcon };
    if (score >= 70)
      return {
        status: "warning",
        color: "yellow",
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              className={`text-lg font-bold text-${health.color}-600 dark:text-${health.color}-400`}
            >
          ...
          <div className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div className="font-semibold text-gray-900 dark:text-gray-100 flex items-center justify-center">
              ...
              {healthData?.active_connections ||
                dashboardData?.active_users_this_week ||
                "0"}
            </div>
            <div className="text-gray-500 dark:text-gray-400">
              Active Connections
        ...
        {(dashboardData?.pending_submissions > 20 ||
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
            ...
            <ul className="mt-2 text-xs text-red-700 dark:text-red-300 space-y-1">
              {dashboardData?.pending_submissions > 20 && (
                <li>
                  • High submission backlog ({dashboardData.pending_submissions}{" "}
                  pending)
                </li>
              )}
              {healthData?.overall_score < 70 && (
...
function DashboardOverview({ data, isLoading, healthData }) {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: CheckCircleIcon,
      color:
        healthData?.overall_score > 90 ? "bg-emerald-500" : "bg-yellow-500",
                ...
                {action.count !== undefined && (
                  <span
                    className={`bg-white bg-opacity-20 px-2 py-1 rounded-full text-xs font-medium ${
                      action.urgent
                        ? "bg-red-100 text-red-800 bg-opacity-100 animate-pulse"
                        : ""
                    }`}
                  >
                    {action.count}
          ...
          {recentActions.map((action, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-900 dark:text-gray-100">
                  <span className="font-medium">{action.admin_username}</span>{" "}
                  {action.action}
                  {action.target_type && (
  ...
  const tabs = useMemo(
    () => [
      ...
      },
      {
        ...
        badgeType:
          dashboardData?.pending_submissions > 10 ? "urgent" : "normal",
