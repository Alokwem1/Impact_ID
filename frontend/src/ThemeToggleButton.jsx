...
export default function ThemeToggleButton({ showDropdown = false, size = 'md' }) {
    const { theme, isSystemTheme, toggleTheme, setLightTheme, setDarkTheme, useSystemTheme } = useTheme();
    ...
    return (
        ...
        >
            <div className="relative">
                {theme === 'dark' ? (
                    <MoonIcon className={`${iconSizes[size]} transform transition-transform duration-200 group-hover:scale-110`} />
                ) : (
                    <SunIcon className={`${iconSizes[size]} transform transition-transform duration-200 group-hover:scale-110 group-hover:rotate-12`} />
                )}
            </div>
        </button>
    );
