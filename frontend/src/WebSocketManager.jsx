...
const createMessageHandlers = (queryClient, user) => ({
  ...
  weaving_update: (data) => {
    ...
    const subtype =
      ...
      data.inner_type ||
      data.kind ||
      (data.type !== "weaving_update" ? data.type : undefined);
    if (data.user_id === user?.id && subtype === "thread_completed") {
      toast.success(
        `Thread woven successfully! +${data.impact_score || 0} impact`,
        {
          icon: "🧵",
          duration: 5000,
        },
  ...
  user_activity: (data) => {
    ...
    queryClient.invalidateQueries({
      queryKey: queryKeys.user.history(user?.id || "current"),
    });
  },
  ...
  streak_update: (data) => {
    ...
    const subtype =
      ...
      data.event_type ||
      data.subtype ||
      (data.type !== "streak_update" ? data.type : undefined);
    if (data.user_id === user?.id) {
...
export function sendAuthRefresh(socketInstance, token) {
  if (!socketInstance || socketInstance.readyState !== WebSocket.OPEN || !token)
    return false;
  try {
...
export default function WebSocketManager() {
  ...
  const messageHandlers = useCallback(
    () => createMessageHandlers(queryClient, user),
    [queryClient, user],
  );
    ...
    try {
      ...
      socketRef.current.onclose = (event) => {
        ...
        if (
          ...
          const delay =
            WS_CONFIG.reconnectDelays[
              Math.min(reconnectAttempts, WS_CONFIG.reconnectDelays.length - 1)
            ];

          console.log(
            `Reconnecting in ${delay}ms (attempt ${reconnectAttempts + 1}/${WS_CONFIG.maxReconnectAttempts})`,
          );
  ...
  useEffect(() => {
    ...
    const unsubRefresh = authEvents.on(
      AUTH_EVENT.TOKEN_REFRESH,
      onTokenRefresh,
    );
    const unsubExpired = authEvents.on(
