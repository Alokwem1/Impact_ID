...
export function useWebSocketStatus() {
  ...
  if (!ctx) throw new Error('useWebSocketStatus must be used within WebSocketStatusProvider');
  return ctx.status;
}
...
export function useWebSocketStatusPublisher() {
  const ctx = useContext(WebSocketStatusContext);
  if (!ctx) throw new Error('useWebSocketStatusPublisher must be used within WebSocketStatusProvider');
  return ctx._update;
}
