...
import { WebSocketStatusProvider, useWebSocketStatus } from './WebSocketStatusContext';
...
class FakeWS {
  ...
  send(){}
  close(evt){ this.onclose && this.onclose({ code: (evt&&evt.code)||1000, reason:'test'}); }
  addEventListener(type, cb){ if(type==='message') this.onmessage = cb; }
  removeEventListener(){}
}
...
describe('WebSocketStatusContext integration', () => {
  ...
  it('reflects connected then disconnected status', async () => {
    ...
    expect(['connecting','connected','disconnected']).toContain(statusNode.textContent);
    // Force close to trigger disconnected
    act(()=>{ __lastSocket && __lastSocket.close({ code:1000 }); });
    await act(()=>Promise.resolve());
