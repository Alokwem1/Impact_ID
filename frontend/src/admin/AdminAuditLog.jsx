...
const fetchAuditLogs = async ({ action, admin_id, days_back, limit, offset }) => {
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-lg">{log.admin_username || `Admin #${log.admin_id}`}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p className="text-lg">{log.target_type} #{log.target_id}</p>
                        </div>
                        <div className="col-span-2">
                            <label className="text-sm font-medium text-gray-500">Timestamp</label>
                            <p className="text-lg">{new Date(log.created_at).toLocaleString()}</p>
                        </div>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-200">
            <div>
                ...
                <select
                    ...
                    onChange={(e) => setAdvancedFilters(prev => ({ ...prev, targetType: e.target.value }))}
            ...
            <div>
                ...
                <input
                    ...
                    onChange={(e) => setAdvancedFilters(prev => ({ ...prev, targetId: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
            </div>
...
export default function AdminAuditLog() {
    ...
    const { 
        ...
        onSuccess: (newLogs) => {
            ...
            if (logs.length > 0 && newLogs.length > 0) {
                ...
                const newCriticalActions = newLogs.filter(log => 
    ...
    const quickFilters = [
        ...
        { label: 'Critical Actions', actions: criticalActions, icon: ExclamationTriangleIcon },
    ...
    const getTargetIcon = useCallback((targetType) => {
        switch (targetType?.toLowerCase()) {
            ...
            case 'task': return <DocumentTextIcon className="h-4 w-4" />;
            case 'submission': return <ShieldCheckIcon className="h-4 w-4" />;
            case 'badge': return <CogIcon className="h-4 w-4" />;
            case 'system': return <BellIcon className="h-4 w-4" />;
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p className="text-sm text-gray-700">
                                ...
                                <span className="font-medium">{Math.min(filters.offset + filters.limit, logs.length)}</span> of{' '}
