...
const flagSubmission = async ({ submissionId, reason, category }) => {
  // ✅ FIXED: Add /api prefix
  const { data } = await apiClient.post(
    `/api/admin/submissions/${submissionId}/flag`,
    {
      reason,
      category,
    },
...
const exportSubmissions = async (filters) => {
  ...
  const response = await apiClient.get(
    `/api/admin/submissions/export?${params}`,
    {
      responseType: "blob",
    },
  );
...
export default function AdminSubmissions() {
  ...
  const bulkReviewMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.detail || "Failed to bulk review submissions.",
      );
    },
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                handleFilterChange("daysBack", parseInt(e.target.value))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
          ...
          {quickFilters.map((filter) => (
            <button
              ...
              onClick={() => {
                ...
                if (filter.daysBack)
                  handleFilterChange("daysBack", filter.daysBack);
              }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setReviewData((prev) => ({
                        ...prev,
                        bonus_xp: parseInt(e.target.value) || 0,
                      }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setReviewData((prev) => ({
                        ...prev,
                        bonus_essence: parseInt(e.target.value) || 0,
                      }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <p className="text-sm text-gray-600 mb-6">
          This action will affect {selectedCount} selected submissions. This
          cannot be undone.
        </p>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFlagData((prev) => ({ ...prev, category: e.target.value }))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setFlagData((prev) => ({ ...prev, reason: e.target.value }))
              }
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
