...
const exportUsers = async (filters) => {
  ...
  const response = await apiClient.get(
    `/api/admin/reports/user-progress?${params}`,
    {
      responseType: "blob",
    },
  );
            ...
            if (activity && activity.length > 0) {
              return (
                <div className="space-y-3">
                  {activity.map((item) => (
                    <div
                      key={`${item.id || item.created_at}-${item.action}`}
...
export default function AdminUserList() {
  ...
  const { data: userActivity, isLoading: activityLoading } = useQuery({
    queryKey: selectedUserActivity?.id
      ? queryKeys.activities.userId(selectedUserActivity.id)
      : ["activities", "user-id", undefined],
  ...
  const bulkStatusMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      toast.success(
        `${variables.userIds.length} users updated to ${variables.status}`,
      );
      queryClient.invalidateQueries({ queryKey: queryKeys.admin.usersBase() });
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  handleFilterChange("include_inactive", e.target.checked)
                }
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="space-y-1">
                        ...
                        <div className="text-sm text-gray-500">
                          {user.task_count || 0} tasks • {user.badge_count || 0}{" "}
                          badges
                        </div>
                      </div>
                    </td>
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <span className="text-sm text-gray-700">
              ...
              {userStats?.total_users &&
                ` of ${userStats.total_users.toLocaleString()}`}
            </span>
            <button
        ...
        {type === "role" && (
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setActionData((prev) => ({ ...prev, role: e.target.value }))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex">
              ...
              <div className="text-sm text-red-800">
                ...
                <p>
                  All user data, submissions, and progress will be permanently
                  deleted.
                </p>
              </div>
            </div>
