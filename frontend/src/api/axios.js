...
apiClient.interceptors.request.use(
  (config) => {
    ...
    // Log request in development (unless suppressed)
    if (isDevelopment && !isTest && !config.suppressLogging) {
      console.log(
...
apiClient.interceptors.response.use(
  (response) => {
    ...
    if (
      ...
      if (
        ...
        !response.config.suppressSuccessToast
      ) {
  ...
  async (error) => {
    ...
    if (isDevelopment && !isTest && !originalRequest?.suppressLogging) {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        authEvents.emit(AUTH_EVENT.TOKEN_REFRESH_FAILED, {
          error: refreshError,
        });
...
apiClient.healthCheck = async () => {
  try {
    // Route through /api/health so Vite dev proxy forwards to backend
...
const testConnection = async () => {
  try {
    if (isDevelopment) {
      console.log(
        "🔍 Testing API connection (attempt %d)...",
        connectionAttempt + 1,
      );
      console.log("📍 Base URL:", API_BASE_URL);
    }

  const healthResponse = await apiClient.get("/api/health", { timeout: 4000 });
    if (isDevelopment)
