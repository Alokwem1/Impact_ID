...
export default function A11yInspector() {
  ...
  const logSummary = (found) => {
    ...
    console.group(
      "%cA11y Inspector Report",
      "background:#2563eb;color:#fff;padding:2px 6px;border-radius:4px;",
    );
    const counts = found.reduce((acc, i) => {
      acc[i.type] = (acc[i.type] || 0) + 1;
      return acc;
    }, {});
    Object.entries(counts).forEach(([k, v]) =>
      console.log(`${k}: %c${v}`, "font-weight:bold"),
    ); // eslint-disable-line no-console
    if (!found.length)
      console.log(
        "%cNo issues detected in current viewport.",
  ...
  return (
    ...
    >
      ...
      >
        {enabled ? "Disable A11y Inspector" : "Enable A11y Inspector"}{" "}
        (Ctrl+Alt+A)
      </button>
      {enabled && (
        <div
