...
export function RouteErrorBoundary({ children }) {
  return (
    <ErrorBoundary
      ...
      onReset={() => {
        /* handled inside fallback via context */
      }}
    >
      {children}
    </ErrorBoundary>
...
function RouteErrorFallback({ error, resetErrorBoundary }) {
  ...
  const isNetworkish = /NetworkError|Failed to fetch/i.test(
    error?.message || "",
  );
  ...
  const handleReport = () => {
    ...
    window.open(
      `mailto:support@impactid.app?subject=App%20Error%20Report&body=${body}`,
      "_blank",
    );
  };
    ...
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-8 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6 text-sm">
          {isChunkLoadError
            ? "A new version may have deployed. Reload to fetch fresh assets."
        ...
        {import.meta.env.DEV && error && (
          <details className="mt-6 text-left">
            ...
            <pre className="mt-2 max-h-40 overflow-auto bg-gray-100 dark:bg-gray-900 text-[11px] p-3 rounded">
              {String(error.stack || error.message)}
            </pre>
            {isNetworkish && (
              <div className="text-amber-600 text-xs mt-2">
                Network hint: check API / connectivity.
              </div>
            )}
          </details>
        )}
