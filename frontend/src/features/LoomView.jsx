...
const THREAD_CATEGORIES = [
    { id: 'Environment', name: 'Environment', icon: GlobeAltIcon, color: 'green' },
    { id: 'Social Good', name: 'Social Good', icon: HeartIcon, color: 'pink' },
    { id: 'Technology', name: 'Technology', icon: ComputerDesktopIcon, color: 'blue' },
    { id: 'Education', name: 'Education', icon: AcademicCapIcon, color: 'purple' },
...
export default function LoomView({ 
    ...
    const [timeRemaining, setTimeRemaining] = useState(status?.time_remaining_seconds || 0);
    ...
    const hasReachedDailyLimit = status?.daily_weaves_completed >= status?.daily_weaves_limit;
    ...
    const getButtonText = () => {
        ...
        if (hasReachedDailyLimit) return `Daily Limit Reached (${status.daily_weaves_limit})`;
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p className="text-purple-100">Transform impact threads into meaningful categorizations</p>
                            </div>
                        </div>
                            ...
                            <div className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full text-sm font-medium ${
                                ...
                                {canWeave ? (
                                    ...
                                    <>
                                        ...
                                        <span>{hasReachedDailyLimit ? 'Daily Limit' : 'Cooling Down'}</span>
                                    </>
                                )}
                            </div>
                        </div>
                    ...
                    {userStats && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center">
                                ...
                                <div className="text-lg font-bold">{userStats.essence_balance}</div>
                                <div className="text-xs text-purple-100">Essence</div>
                            </div>
                            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center">
                                <BoltIconSolid className="h-5 w-5 mx-auto mb-1 text-blue-300" />
                                <div className="text-lg font-bold">{userStats.daily_weaves_completed}/{userStats.daily_weaves_limit}</div>
                            ...
                            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center">
                                ...
                                <div className="text-lg font-bold">{userStats.total_weaves || 0}</div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    {!canWeave && !hasReachedDailyLimit && (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <h4 className="font-semibold text-yellow-800">Weaving Cooldown Active</h4>
                                    <p className="text-yellow-700">
                                        Next weaving available in <span className="font-bold">{formatTime(timeRemaining)}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {hasReachedDailyLimit && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <h4 className="font-semibold text-blue-800">Daily Weaving Limit Reached</h4>
                                    <p className="text-blue-700">
                                        You've completed {status.daily_weaves_limit} weaves today. Come back tomorrow for more!
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {showThreads && canWeave && (
                        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div className="flex items-center justify-between mb-4">
                                <h4 className="font-semibold text-gray-900">Available Threads ({status.available_threads || 0})</h4>
                                <div className="flex items-center space-x-2">
                                    ...
                                    >
                                        <ArrowPathIcon className={`h-4 w-4 ${threadsLoading ? 'animate-spin' : ''}`} />
                                    </button>
                                </div>
                            </div>
                                    ...
                                    {availableThreads.map(thread => (
                                        ...
                                        >
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <h5 className="font-medium text-gray-900 mb-1 line-clamp-2">
                                                        {thread.title || thread.meta_data?.title || 'Impact Thread'}
                                                    </h5>
                                                    {thread.summary && (
                                            ...
                                            <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                                                ...
                                                {thread.category && (
                                                    <span className={`px-2 py-1 rounded-full ${getCategoryColor(thread.category)}`}>
                                                        {thread.category}
                                                    </span>
                                                )}
                                            </div>
                            ...
                            {leaderboard.length > 0 ? (
                                <div className="space-y-2">
                                    {leaderboard.slice(0, 5).map((entry, index) => (
                                        <div key={entry.username} className="flex items-center space-x-3 p-2 bg-white rounded-lg">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="text-xs text-gray-500">
                                                    {entry.weave_count} weaves • {entry.total_essence} essence
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <p className="text-gray-500 text-center py-4">Leaderboard loading...</p>
                            )}
                        </div>
                    )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                        <div className="text-2xl font-bold text-purple-600">{status.available_threads || 0}</div>
                    ...
                    <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">{userStats?.streak || 0}</div>
                        <div className="text-sm text-gray-600">Current Streak</div>
                    </div>
                    <div className="text-center">
                        <div className="text-2xl font-bold text-red-600">{userStats?.total_weaves || 0}</div>
