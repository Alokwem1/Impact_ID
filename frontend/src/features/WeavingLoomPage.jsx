...
const fetchAvailableThreads = async ({
  ...
  try {
    const { data } = await apiClient.get(
      `/api/weaving/available-threads?${params.toString()}`,
    );
    return data;
  } catch (error) {
...
const submitWeaving = async (submissionData) => {
  const { thread_id, category, reasoning, action_plan, difficulty_rating } =
    submissionData;
...
const fetchWeavingLeaderboard = async ({
  period = "weekly",
  limit = 10,
} = {}) => {
  try {
    const { data } = await apiClient.get(
      `/api/weaving/leaderboard?period=${period}&limit=${limit}`,
    );
    return data;
  } catch (error) {
...
const fetchUserProfile = async () => {
  try {
    // Corrected: backend route is /api/users/@me
...
export default function WeavingLoomPage() {
  ...
  const {
    ...
    queryKey: [
      ...queryKeys.weaving.availableThreads(),
      selectedCategory,
      qualityFilter,
    ],
    queryFn: () =>
  ...
  const { data: leaderboard = [], isLoading: leaderboardLoading } = useQuery({
    ...
    queryFn: () =>
      fetchWeavingLeaderboard({ period: leaderboardPeriod, limit: 20 }),
    staleTime: 5 * 60 * 1000,
    retry: 2,
  });
  ...
  const { mutate: handleClaim, isPending: isClaiming } = useMutation({
    ...
    onError: (err) => {
      const errorMessage =
        ...
        "Failed to claim thread. Please try again.";
      ...
      if (err.response?.status === 429) {
        refetchStatus();
        toast.info(
          "Cooldown active. Please wait before claiming another thread.",
          {
            icon: "⏳",
            duration: 3000,
          },
        ...
        refetchThreads();
        toast.info(
          "Thread no longer available. Refreshing available threads...",
          {
            icon: "🔄",
            duration: 3000,
          },
  ...
  const { mutate: handleSubmit, isPending: isSubmitting } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.streak && [7, 30, 100].includes(data.streak)) {
        setTimeout(() => {
          toast.success(
            `🔥 ${data.streak} Day Streak Milestone! You're on fire!`,
            {
              icon: "🔥",
              duration: 5000,
              style: {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.weaving.availableThreads(),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.weaving.leaderboard(),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.weaving.analytics(),
      });
    ...
    onError: (err) => {
      const errorMessage =
        err.response?.data?.detail ||
        err.message ||
        "Failed to submit weaving. Please try again.";
      toast.error(errorMessage, {
        icon: "❌",
        duration: 5000,
      });
      ...
      if (err.message.includes("characters long")) {
        toast.info(
          "💡 Tip: Provide more detailed reasoning for better rewards!",
          {
            duration: 4000,
          },
        );
  ...
  const renderError = useCallback(() => {
    ...
    const isNetworkError =
      statusErrorMessage?.message?.includes("Network Error") ||
        ...
        <p className="text-red-700 dark:text-red-300 mb-4">
          {isNetworkError
            ...
            : statusErrorMessage?.message ||
              "Could not load the weaving experience."}
        </p>
      ...
      case "weaving":
        ...
        return (
          <WeavingView
            ...
            estimatedReward={
              WEAVING_MODES[showAdvancedMode ? "advanced" : "quick"].baseReward
            }
            ...
            {leaderboardLoading ? (
              <div className="space-y-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className="animate-pulse flex items-center space-x-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"
                  >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
          ...
          {userStats ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-lg p-6 text-center border border-blue-200 dark:border-blue-700">
                  ...
                  <div className="text-xs text-blue-500 dark:text-blue-400 mt-1">
                    ...
                    {userStats.daily_weaves_limit} today
                  </div>
                </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    {
                      milestone: 10,
                      achieved: userStats.total_weaves >= 10,
                      label: "First 10 Weaves",
                    },
                    {
                      milestone: 50,
                      achieved: userStats.total_weaves >= 50,
                      label: "50 Weaves",
                    },
                    {
                      milestone: 7,
                      achieved: userStats.streak >= 7,
                      label: "7-Day Streak",
                    },
                    {
                      milestone: 1000,
                      achieved: userStats.essence_balance >= 1000,
                      label: "1K Essence",
                    },
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Loading your weaving statistics...
              </p>
            </div>
          )}
        </div>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-2xl font-bold text-indigo-600">
                      {analytics.total_threads_woven || 0}
                    </div>
    ...
    () => [
      {
        ...
        label: "Weaving Loom",
        icon: GlobeAltIcon,
        description: "Claim and weave threads",
      },
      {
        key: "leaderboard",
        label: "Leaderboard",
        icon: TrophyIcon,
        description: "Top weavers rankings",
      },
      {
        key: "stats",
        label: "My Stats",
        icon: ChartBarIcon,
        description: "Your weaving progress",
      },
      {
        key: "analytics",
        label: "Analytics",
          ...
          <div className="bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                ...
                <p className="text-purple-100 text-lg">
                  Transform raw impact threads into meaningful categorizations
                  and earn essence
                </p>
              </div>
            ...
            <div className="flex space-x-1 bg-white bg-opacity-20 rounded-lg p-1">
              {navigationTabs.map((tab) => {
                ...
                const isDisabled =
                  viewMode === "weaving" && tab.key !== "weaving";
                ...
                return (
                  <button
                    ...
                    title={
                      isDisabled ? "Finish weaving to access" : tab.description
                    }
                  >
                ...
                <div className="flex items-center space-x-4">
                    ...
                    {Object.entries(WEAVING_MODES).map(([mode, config]) => (
                      <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${
                          (showAdvancedMode && mode === "advanced") ||
                          (!showAdvancedMode && mode === "quick")
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400 max-w-xs">
                      {
                        WEAVING_MODES[showAdvancedMode ? "advanced" : "quick"]
                          .description
                      }
                    </span>
                  </div>
