...
export default function WeavingView({ 
    ...
    useEffect(() => {
        ...
        if (reasoning.includes('because') || reasoning.includes('due to')) score += 1;
    ...
    const handleSubmit = async (categoryId, quickSubmit = false) => {
        ...
        const submissionData = {
            thread_id: thread.id,
            category: categoryId,
            reasoning: quickSubmit ? `Quick categorization as ${categoryId}` : reasoning.trim(),
        ...
        try {
            ...
            // Success feedback
            toast.success(`Weaving submitted! Estimated reward: ${estimatedReward.min}-${estimatedReward.max} essence`, {
                icon: '✨',
                duration: 4000
            });
        } catch (error) {
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-purple-100 text-sm">Help categorize this content to earn essence</p>
                        </div>
                    </div>
                ...
                {userStats && (
                    <div className="grid grid-cols-3 gap-4 text-center">
                        <div className="bg-white bg-opacity-20 rounded-lg p-3">
                            <div className="text-lg font-semibold">{userStats.essence_balance}</div>
                            <div className="text-xs text-purple-100">Current Essence</div>
                        </div>
                        <div className="bg-white bg-opacity-20 rounded-lg p-3">
                            <div className="text-lg font-semibold">{userStats.daily_weaves_completed}/{userStats.daily_weaves_limit}</div>
                            <div className="text-xs text-purple-100">Daily Weaves</div>
                        </div>
                        <div className="bg-white bg-opacity-20 rounded-lg p-3">
                            <div className="text-lg font-semibold">{userStats.streak || 0}</div>
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                            <h4 className="font-semibold text-gray-900 mb-2 leading-relaxed">
                                {thread.title || thread.meta_data?.title || 'Impact Thread Content'}
                            </h4>
                            {thread.summary && (
                    ...
                    {showDetails && (
                        <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span className="ml-2 text-gray-900">{thread.source || 'Web'}</span>
                            </div>
                            <div>
                                <span className="text-gray-500">Type:</span>
                                <span className="ml-2 text-gray-900">{thread.data_type || 'URL'}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span className="ml-2 text-gray-900">{thread.weaving_count || 0}</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {WEAVING_CATEGORIES.map(category => {
                            const Icon = selectedCategory === category.id ? category.solidIcon : category.icon;
                ...
                {showAdvanced && selectedCategory && (
                    <div className="bg-blue-50 rounded-xl p-5 border border-blue-200 space-y-4">
                        <h6 className="font-semibold text-blue-900 mb-3">Advanced Weaving Options</h6>
                        
                        {/* Reasoning */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                Why does this belong in "{selectedCategory}"? (min 20 characters)
                            </label>
                            <textarea
                                value={reasoning}
                            ...
                            <div className="flex items-center justify-between mt-1 text-xs">
                                <span className={`${reasoning.length >= 20 ? 'text-green-600' : 'text-gray-500'}`}>
                ...
                {!showAdvanced && (
                    <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
                        <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span className="text-sm font-medium text-yellow-800">Quick Weaving</span>
                        </div>
                        <p className="text-xs text-yellow-700 mb-3">
                            Select a category and submit quickly for reduced but immediate rewards.
                        </p>
                        {selectedCategory && (
                            <button
                            ...
                            >
                                {submitting ? 'Submitting...' : `Quick Weave as ${selectedCategory}`}
                            </button>
                        )}
                    </div>
                )}
                    ...
                    {showAdvanced ? (
                        <>
                            <button
                                ...
                                disabled={submitting || !selectedCategory || reasoning.length < 20}
