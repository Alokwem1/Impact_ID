...
const fetchUserProfile = async () => {
  // Backend exposes /api/users/@me (not /api/users/me)
...
const fetchNotificationsCount = async () => {
  try {
    // Backend defines GET /api/notifications/unread/count returning { "unread_count": <int> }
    const { data } = await apiClient.get("/api/notifications/unread/count");
    return data.unread_count || 0;
  } catch (error) {
    return 0; // Gracefully degrade if endpoint not available
  }
};
...
export default function Layout({
  ...
  const isActiveLink = (path) => {
    ...
    if (path === "/tasks" && location.pathname.startsWith("/tasks"))
      return true;
    if (path === "/badges" && location.pathname.startsWith("/badges"))
      return true;
    if (path === "/leaderboard" && location.pathname.startsWith("/leaderboard"))
      return true;
    if (path === "/activities" && location.pathname.startsWith("/activities"))
      return true;
    if (path === "/weaving" && location.pathname.startsWith("/weaving"))
      return true;
