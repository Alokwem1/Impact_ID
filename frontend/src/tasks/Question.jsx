...
export default function Question({ 
    ...
    const handleSubmit = (autoSubmit = false) => {
        ...
        const isCorrect = selectedOption === question.correct_answer || 
                         (question.options && selectedOption === question.options.find(opt => opt.is_correct)?.text);
    ...
    const getOptionClasses = (option) => {
        const baseClasses = 'relative block p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 transform hover:scale-[1.02]';
        ...
        if (isSubmitted) {
            const isCorrect = option === question.correct_answer || 
                            (question.options && question.options.find(opt => opt.text === option)?.is_correct);
            ...
            <div className="space-y-3 mb-6">
                {questionOptions.length > 0 ? (
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                        <p className="text-gray-600">This question requires a text response.</p>
                        <textarea
