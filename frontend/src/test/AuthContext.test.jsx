...
function setToken(token, remember) {
  if (remember) localStorage.setItem('accessToken', token); else sessionStorage.setItem('accessToken', token);
}
...
describe('AuthContext', () => {
  ...
  it('loads user when token present', async () => {
    setToken('abc', false);
    apiClient.get.mockResolvedValueOnce({ data: { username: 'alice', role: 'user' } });
    renderWithProviders(<Echo />);
  ...
  it('login stores token (rememberMe=false -> sessionStorage)', async () => {
    ...
    apiClient.post.mockResolvedValue({ data: { access_token: 'tok123', username: 'bob' } }); // any number of invocations
    apiClient.get.mockResolvedValue({ data: { username: 'bob', role: 'user' } }); // subsequent fetchUser calls

    function LoginProbe() {
      const { login } = useAuth();
      React.useEffect(() => { login('Bob', 'pw'); }, [login]);
      return <Echo />;
    }
  ...
  it('login stores token in localStorage when rememberMe=true', async () => {
    // No initial /me call expected (no token yet) so directly mock login then user fetch
    apiClient.post.mockResolvedValue({ data: { access_token: 'tok456', username: 'carol' } });
    apiClient.get.mockResolvedValue({ data: { username: 'carol', role: 'user' } });

    function LoginProbe() {
      const { login } = useAuth();
      React.useEffect(() => { login('Carol', 'pw', true); }, [login]);
      return <Echo />;
    }
  ...
  // Initial with token
  setToken('abc', false);
  apiClient.get.mockResolvedValueOnce({ data: { username: 'dave', role: 'user' } }); // first fetchUser
  apiClient.post.mockResolvedValueOnce({ data: {} }); // logout endpoint

    function LogoutProbe() {
      const { logout, isAuthenticated } = useAuth();
      React.useEffect(() => { if (isAuthenticated) logout(); }, [isAuthenticated, logout]);
      return <Echo />;
    }

    renderWithProviders(<LogoutProbe />);
    await waitFor(() => expect(screen.getByTestId('isAuth').textContent).toBe('false'));
    expect(localStorage.getItem('accessToken')).toBeNull();
    expect(sessionStorage.getItem('accessToken')).toBeNull();
  });
