...
describe('WebSocketManager activity events', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(AuthModule, 'useAuth').mockReturnValue({ user: { id: 1, role: 'user' }, isAuthenticated: true });
  ...
  it('invalidates activities cache on new_activity and reaction_update', async () => {
    ...
    await act(async () => {
      wsInstance.onmessage?.({ data: JSON.stringify({ type: 'new_activity', username: 'alice', action: 'task_completed' }) });
    });

    // Fire reaction_update event
    await act(async () => {
      wsInstance.onmessage?.({ data: JSON.stringify({ type: 'reaction_update', activity_id: 10 }) });
    });
