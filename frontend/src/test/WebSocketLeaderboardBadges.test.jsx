...
function mountWithClient(client) {
  return renderWithProviders(<WebSocketManager />, { includeAuth: false, queryClient: client });
}
...
describe('WebSocketManager leaderboard & badges updates', () => {
  beforeEach(() => {
    ...
    vi.spyOn(AuthModule, 'useAuth').mockReturnValue({ user: { id: 7, role: 'user' }, isAuthenticated: true });
  ...
  it('handles leaderboard_update: invalidates leaderboard + weaving leaderboard and shows rank toast when user matches', async () => {
    ...
    await waitFor(() => {
      ...
      const hasLeaderboardRoot = invalidateSpy.mock.calls.some(call => call[0]?.queryKey?.[0] === queryKeys.leaderboard.root()[0]);
      ...
      const hasWeavingLeaderboard = invalidateSpy.mock.calls.some(call => {
        ...
        return Array.isArray(qk) && qk[0] === 'weaving' && qk[1] === 'leaderboard';
      });
      expect(hasWeavingLeaderboard).toBe(true);
      // Toast for rank movement
  const toastCalled = toast.success.mock.calls.some(c => String(c[0]).includes('#3'));
  ...
  it('handles badges_update: invalidates badges root + user.me + achievements.recent and shows badge toast', async () => {
    ...
    const payload = { type: 'badges_update', user_id: 7, badge_title: 'Explorer' };
    await act(async () => { wsInstance.onmessage?.({ data: JSON.stringify(payload) }); });
    ...
    await waitFor(() => {
      ...
      const containsKey = (target) => keysSeen.some(k => Array.isArray(k) && k.length === target.length && k.every((v,i) => v === target[i]));
  ...
  const toastCalled = toast.success.mock.calls.some(c => String(c[0]).includes('Explorer'));
      expect(toastCalled).toBe(true);
    });
  });
});
