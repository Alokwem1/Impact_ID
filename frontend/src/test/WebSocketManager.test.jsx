...
describe('WebSocketManager auth event reactions', () => {
  ...
  it('sends auth_refresh with new token on TOKEN_REFRESH', async () => {
    ...
    await act(async () => { authEvents.emit(AUTH_EVENT.TOKEN_REFRESH, { token: 'refreshed123' }); });
    await waitFor(() => {
      const sent = wsInstance.sent.find(m => { try { const p = JSON.parse(m); return p.type === 'auth_refresh' && p.token === 'refreshed123'; } catch { return false; } });
  ...
  it('closes socket with code 4001 on SESSION_EXPIRED', async () => {
    ...
    await act(async () => { authEvents.emit(AUTH_EVENT.SESSION_EXPIRED, { reason: 'expired' }); });
    // Prefer observable side-effect: console.warn invoked with 4001 + session_expired
    await waitFor(() => {
      const saw4001 = warnSpy.mock.calls.some(call => call.join(' ').includes('WebSocket disconnected: 4001'));
  ...
  it('cleanup executed on LOGOUT (socket closed, no further sends)', async () => {
    ...
    await act(async () => { authEvents.emit(AUTH_EVENT.LOGOUT, {}); });
    await waitFor(() => {
      expect(wsInstance.readyState).toBe(READY.CLOSED);
    });
    const prevSend = wsInstance.sent.length;
    await act(async () => { authEvents.emit(AUTH_EVENT.TOKEN_REFRESH, { token: 'after-logout' }); });
