...
const containsKey = (calls, target) => calls.some(k => Array.isArray(k) && k.length === target.length && k.every((v,i)=>v===target[i]));
...
describe('WebSocketManager rewards & streak events', () => {
  beforeEach(() => {
    ...
    vi.spyOn(AuthModule, 'useAuth').mockReturnValue({ user: { id: 555, role: 'user' }, isAuthenticated: true });
  ...
  it('essence_update invalidates user.me & dashboard and shows success toast when amount > 0', async () => {
    ...
    await act(async () => { mountWithClient(qc); });
    ...
    await waitFor(() => {
      const keysSeen = invalidateSpy.mock.calls.map(c => c[0]?.queryKey).filter(Boolean);
      expect(containsKey(keysSeen, queryKeys.user.me())).toBe(true);
      expect(containsKey(keysSeen, queryKeys.user.dashboard())).toBe(true);
  const toastCalled = toast.success.mock.calls.some(c => /12.*Essence/i.test(c[0]));
  ...
  it('level_up invalidates user.me & dashboard and shows level toast', async () => {
    ...
    await waitFor(() => {
      ...
      expect(containsKey(keysSeen, queryKeys.user.me())).toBe(true);
      expect(containsKey(keysSeen, queryKeys.user.dashboard())).toBe(true);
  const toastCalled = toast.success.mock.calls.some(c => /level 8/i.test(c[0]));
      expect(toastCalled).toBe(true);
    });
  });
  ...
  it('streak_update milestone vs lost: different toast channels and invalidations', async () => {
    ...
    await act(async () => { wsInstance.readyState = READY.OPEN; wsInstance.onopen?.(); });
  // milestone
  await act(async () => { wsInstance.onmessage?.({ data: JSON.stringify({ type: 'streak_update', user_id: 555, streak_count: 5, streak_type: 'streak_milestone' }) }); });
  // lost
  await act(async () => { wsInstance.onmessage?.({ data: JSON.stringify({ type: 'streak_update', user_id: 555, streak_type: 'streak_lost' }) }); });
    await waitFor(() => {
      const keysSeen = invalidateSpy.mock.calls.map(c => c[0]?.queryKey).filter(Boolean);
      expect(containsKey(keysSeen, queryKeys.user.me())).toBe(true);
      expect(containsKey(keysSeen, queryKeys.user.dashboard())).toBe(true);
      // success toast for milestone
  const milestoneToast = toast.success.mock.calls.some(c => /streak/i.test(c[0]));
      expect(milestoneToast).toBe(true);
      // error toast for lost
  const lostToast = toast.error.mock.calls.some(c => /Streak lost/i.test(c[0]));
