...
describe('Accessibility smoke (Register & Dashboard)', () => {
  it('RegisterPage has no serious/critical violations', async () => {
    ...
    if (serious.length) {
      console.error('RegisterPage a11y serious violations', JSON.stringify(serious, null, 2));
    }
  ...
  it('DashboardPage scaffold has no serious/critical violations', async () => {
    ...
    localStorage.setItem('user', JSON.stringify({ id: 1, username: 'demo', role: 'user' }));
    const { container } = shellRender('/dashboard', <DashboardPage />);
    const results = await axe(container);
    const serious = results.violations.filter(v => ['serious','critical'].includes(v.impact));
    if (serious.length) {
      console.error('DashboardPage a11y serious violations', JSON.stringify(serious, null, 2));
    }
    expect(serious).toHaveLength(0);
  });
});
