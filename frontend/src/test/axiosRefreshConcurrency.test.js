...
describe("axios refresh concurrency", () => {
  ...
  it("only performs a single refresh for concurrent 401 responses and replays queued requests", async () => {
    ...
    const originalConfigA = {
      url: "/api/protected/a",
      method: "get",
      headers: {},
    };
    const originalConfigB = {
      url: "/api/protected/b",
      method: "get",
      headers: {},
    };
    const originalConfigC = {
      url: "/api/protected/c",
      method: "get",
      headers: {},
    };
    ...
    apiClient.request = vi.fn(async (cfg) => ({
      status: 200,
      data: { ok: true, path: cfg.url },
      config: {
        ...cfg,
        headers: { ...(cfg.headers || {}), Authorization: "Bearer new_token" },
      },
    }));
    ...
    expect(results.map((r) => r.config.headers.Authorization)).toEqual([
      "Bearer new_token",
      "Bearer new_token",
      "Bearer new_token",
    ]);
