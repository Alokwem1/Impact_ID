...
const exportBadgeCollection = async (format = "pdf") => {
  try {
    const response = await apiClient.get(
      `/api/badges/my/export?format=${format}`,
      {
        responseType: "blob",
      },
    );
...
export default function BadgeList() {
      ...
      if (data.awarded && data.awarded.length > 0) {
        ...
        if (data.awarded.length > 1) {
          setTimeout(
            () => {
              toast.success(
                `🌟 Amazing! You earned ${data.awarded.length} new badges!`,
                {
                  duration: 8000,
                  icon: "🌟",
                },
  ...
  const getCategoryConfig = useCallback((category) => {
    return (
      Object.values(BADGE_CATEGORIES).find((cat) => cat.value === category) ||
      BADGE_CATEGORIES.TASKS
    );
  }, []);
  ...
  const getRarityConfig = useCallback((rarity) => {
    return (
      Object.values(BADGE_RARITIES).find((r) => r.value === rarity) ||
      BADGE_RARITIES.COMMON
    );
  }, []);
      ...
      <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 rounded-xl p-6 text-white shadow-lg">
        ...
        {badgeStats && !statsLoading && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center backdrop-blur-sm">
              <div className="text-2xl font-bold">
                {Math.round(badgeStats.completion_percentage)}%
              </div>
              <div className="text-sm text-blue-100">Collection Complete</div>
              <div className="text-xs text-blue-200 mt-1">
                {badgeStats.total_badges_available -
                  badgeStats.total_badges_earned}{" "}
                remaining
              </div>
            </div>
            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center backdrop-blur-sm">
              ...
              <div className="text-xs text-blue-200 mt-1">Platform total</div>
            </div>
            <div className="bg-white bg-opacity-20 rounded-lg p-4 text-center backdrop-blur-sm">
              <div className="text-2xl font-bold">
        ...
        {statsLoading && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="bg-white bg-opacity-20 rounded-lg p-4 animate-pulse"
              >
            ...
            {[
              {
                ...
                count: badgeStats?.total_badges_earned,
              },
              {
                key: "available",
                label: "Available",
              ...
              },
              {
                ...
                label: "All Badges",
                icon: Squares2X2Icon,
                count: badgeStats?.total_badges_available,
              },
            ].map((tab) => {
              const Icon = tab.icon;
              return (
              ...
              {activeTab !== "earned" && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateFilter("rarity", e.target.value || null)
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100"
                  >
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <div className="flex items-center justify-between">
              <span className="text-sm text-blue-800 dark:text-blue-200">
                {selectedBadges.size} badge
                {selectedBadges.size !== 1 ? "s" : ""} selected
              </span>
              <div className="flex items-center space-x-2">
                <button
      ...
      {badgeStats && activeTab === "earned" && !statsError && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-6">
            ...
            {/* Rarity Breakdown */}
            {badgeStats.rarity_breakdown &&
              Object.keys(badgeStats.rarity_breakdown).length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                    Badges by Rarity
                  </h4>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {Object.entries(badgeStats.rarity_breakdown).map(
                      ([rarity, count]) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`font-bold text-xl text-${rarityConfig.color}-600 dark:text-${rarityConfig.color}-400`}
                            >
                              {count}
                            </div>
                            <div
                              className={`text-xs capitalize text-${rarityConfig.color}-600 dark:text-${rarityConfig.color}-400`}
                            >
                              {rarity}
                            </div>
                          </div>
                    ...
                    <div className="mt-2">
                      ...
                      <div className="w-full bg-yellow-200 dark:bg-yellow-800 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${badgeStats.next_milestone.progress}%`,
                          }}
                        />
                      </div>
                    </div>
