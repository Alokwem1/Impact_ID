...
export default function Leaderboard() {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.detail || "Could not load the leaderboard.";
  ...
  const fetchStats = useCallback(async () => {
    try {
      const [statsResponse, achievementsResponse] = await Promise.all([
        ...
        apiClient.get("/api/leaderboard/recent-achievements", {
          params: { limit: 5 },
        }),
      ]);
  ...
  const formatAdditionalData = (data, type) => {
    ...
    const formatters = {
      ...
      streak: {
        last_task_date: (value) =>
          `Last: ${new Date(value).toLocaleDateString()}`,
      },
      weaving: {
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4">
          <div className="flex items-center space-x-3 mb-4 sm:mb-0">
            <CurrentIconSolid
              className={`h-8 w-8 ${currentTypeConfig.color}`}
            />
            <div>
        ...
        {userPosition && (
          <div
            className={`p-3 rounded-lg border ${currentTypeConfig.bgColor} ${currentTypeConfig.borderColor}`}
          >
            <div className="flex items-center justify-between">
              <p className={`text-sm font-medium ${currentTypeConfig.color}`}>
                Your Position: #{userPosition.position} of{" "}
                {userPosition.total_users}
              </p>
              <p className="text-xs text-gray-600">
                {userPosition.percentile &&
                  `Top ${(100 - userPosition.percentile).toFixed(1)}%`}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {stats.total_tasks_completed?.toLocaleString() || "0"}
              </p>
              <p className="text-xs text-gray-600">Tasks Completed</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">
                {stats.average_xp?.toFixed(0) || "0"}
              </p>
