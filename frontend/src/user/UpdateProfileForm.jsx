...
export default function UpdateProfileForm() {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(async () => {
            if (profileData.username && profileData.username !== user?.username && profileData.username.length >= 3) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(async () => {
            if (profileData.email && profileData.email !== user?.email && profileData.email.includes('@')) {
                setCheckingAvailability(true);
    ...
    const avatarMutation = useMutation({
        ...
        onError: (err) => {
            ...
            const errorMessage = err.response?.data?.detail || 'Failed to upload avatar.';
            toast.error(errorMessage);
            setAvatarPreview(null);
        }
    });
    ...
    const handleProfileSubmit = (e) => {
        ...
        if (profileData.username !== originalUser.username) payload.username = profileData.username;
        if (profileData.email !== originalUser.email) payload.email = profileData.email;
        if (profileData.bio !== (originalUser.bio || '')) payload.bio = profileData.bio;
        if (profileData.location !== (originalUser.location || '')) payload.location = profileData.location;
        if (profileData.website !== (originalUser.website || '')) payload.website = profileData.website;
        
        // Check if social links changed
        const originalSocial = originalUser.social_links || {};
        if (JSON.stringify(profileData.social_links) !== JSON.stringify(originalSocial)) {
        ...
        if (JSON.stringify(profileData.privacy_settings) !== JSON.stringify(originalPrivacy)) {
            ...
            <div className="bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-4">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-blue-100 mt-1">Manage your profile, security, and privacy settings</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-6">
                    ...
                    <form onSubmit={handleProfileSubmit} className="space-y-6">
                        ...
                        <div className="flex items-center space-x-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">Upload a new avatar (max 5MB, JPG/PNG)</p>
                                    ...
                                    <div key={platform}>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                value={url}
                                                onChange={handleProfileChange}
                                                className={getInputClasses(`social_${platform}`, true)}
                                        ...
                                        {errors[`social_${platform}`] && (
                                            <p className="text-xs text-red-500 mt-1">{errors[`social_${platform}`]}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                        ...
                        <button
                            type="submit"
                            disabled={profileMutation.isPending || checkingAvailability || usernameAvailable === false || emailAvailable === false}
                ...
                {activeTab === 'security' && (
                    <form onSubmit={handlePasswordSubmit} className="space-y-6">
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <ul className="text-xs text-blue-800 space-y-1">
                                <li className="flex items-center space-x-2">
                                    <CheckCircleIcon className={`h-4 w-4 ${passwordData.new_password.length >= 8 ? 'text-green-500' : 'text-gray-400'}`} />
                                    <span>At least 8 characters long</span>
                                </li>
                                <li className="flex items-center space-x-2">
                                    <CheckCircleIcon className={`h-4 w-4 ${/[A-Z]/.test(passwordData.new_password) ? 'text-green-500' : 'text-gray-400'}`} />
                                ...
                                <li className="flex items-center space-x-2">
                                    <CheckCircleIcon className={`h-4 w-4 ${/\d/.test(passwordData.new_password) ? 'text-green-500' : 'text-gray-400'}`} />
                                    <span>Contains number</span>
                                </li>
                                <li className="flex items-center space-x-2">
                                    <CheckCircleIcon className={`h-4 w-4 ${/[!@#$%^&*(),.?":{}|<>]/.test(passwordData.new_password) ? 'text-green-500' : 'text-gray-400'}`} />
                        ...
                        <button
                            type="submit"
                            disabled={passwordMutation.isPending || passwordStrength.level === 'weak'}
                ...
                {activeTab === 'privacy' && (
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            {Object.entries(profileData.privacy_settings).map(([setting, enabled]) => (
                                <div key={setting} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                                    <div className="flex-1">
                                        <h4 className="font-medium text-gray-900 capitalize">
                                            {setting.replace('show_', '').replace('_', ' ')}
                                        </h4>
                                        <p className="text-sm text-gray-600 mt-1">
                                            {setting === 'show_email' && 'Display your email address on your public profile'}
                                            {setting === 'show_location' && 'Show your location to other users'}
                                            {setting === 'show_streak' && 'Display your current streak on your profile'}
                                            {setting === 'show_badges' && 'Show your earned badges to other users'}
                                        </p>
                                    </div>
                            ...
                            <div className="flex items-start space-x-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-yellow-700 mt-1">
                                        These settings control your public profile visibility. Some information may still be visible to administrators for platform functionality.
                                    </p>
                                </div>
                            </div>
                        </div>
