...
const HistoryItem = ({
  ...
  const activityConfig =
    ACTIVITY_TYPES[activity.action] || ACTIVITY_TYPES.task_completed;
  ...
  return (
    <div
      className={`bg-white p-4 rounded-xl shadow-sm border-l-4 ${activityConfig.borderColor} hover:shadow-md transition-shadow duration-200`}
    >
      <div className="flex items-start space-x-3">
        {/* Activity Icon */}
        <div
          className={`p-2 rounded-full ${activityConfig.bgColor} flex-shrink-0`}
        >
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2 mt-1">
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${activityConfig.bgColor} ${activityConfig.color}`}
                >
              ...
              {/* Metadata */}
              {activity.meta_data &&
                Object.keys(activity.meta_data).length > 0 && (
          ...
          {onReact && (
            <div className="flex items-center space-x-2 mt-3 pt-2 border-t border-gray-100">
              {Object.entries(REACTION_TYPES).map(([type, config]) => {
                const Icon =
                  currentUserReaction === type ? config.iconSolid : config.icon;
          ...
          prev.map((activity) => {
            if (activity.id === activityId) {
              ...
              if (response.data.is_removed) {
                ...
                // Add new reaction
                updatedActivity.reaction_counts[reactionType] =
                  (updatedActivity.reaction_counts[reactionType] || 0) + 1;
                updatedActivity.user_reaction = reactionType;
              }
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="text-blue-700">
              ...
              <span className="ml-1 font-semibold">
                ({stats.most_active_count} activities)
              </span>
            </div>
          </div>
        </div>
